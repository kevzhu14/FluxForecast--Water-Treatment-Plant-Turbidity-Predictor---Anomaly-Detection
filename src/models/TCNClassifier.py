...
from src.pipeline.DataProcessing import DataSplit, SlidingWindowWithTarget, WindowsToNmp, DataProcessing, NormalizeStd, DropNaNCols, ApplyNaNDrop
...
def TrainModelTCNClassifier(
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Metric")
    plt.title("Training Classification Metrics - TCN")
    plt.legend()
    plt.tight_layout()
    plt.savefig("training_classification_metrics_tcn.png", dpi=300)
    plt.close()
...
def main():
    ...
    train_data = SlidingWindowWithTarget(
        X_train_df, window_size, shift_step, threshold=threshold, horizon=horizon
    )
    val_data = SlidingWindowWithTarget(
        X_val_df, window_size, shift_step, threshold=threshold, horizon=horizon
    )
    test_data = SlidingWindowWithTarget(
        X_test_df, window_size, shift_step, threshold=threshold, horizon=horizon
    )
    ...
    X_train_seq, X_val_seq, X_test_seq, scaler = NormalizeStd(
